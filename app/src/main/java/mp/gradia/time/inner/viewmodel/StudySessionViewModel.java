...
public class StudySessionViewModel extends ViewModel {
    // 로깅 태그
    ...
    /**
     * StudySessionViewModel의 생성자입니다. StudySessionDao를 인자로 받아 과목 목록을 로드합니다.
     */
    public StudySessionViewModel(StudySessionDao sessionDao) {
        this.sessionDao = sessionDao;
        this.subjectDao = null;
    }

    /**
     * StudySessionViewModel의 생성자입니다. StudySessionDao와 SubjectDao를 인자로 받습니다.
     */
    public StudySessionViewModel(StudySessionDao sessionDao, SubjectDao subjectDao) {
        this.sessionDao = sessionDao;
                        ...
                        .subscribe(
                                scheduleDataBundle -> {
                                    ...
                                    for (StudySessionEntity session : scheduleSession) {
                                        Log.d("SessionStatisticalDialog", session.getSubjectName() + " " + session.getDate() + " " + session.getDate() + " " + session.getStartTime());
                                    }
     ...
     */
    private void addEventItem(List<ScheduleEventItem> items, StudySessionEntity session, LocalTime startTime, LocalTime endTime, int eventColor) {
        if (startTime.isAfter(endTime) || startTime.equals(endTime) || (startTime.equals(LocalTime.MIN) && endTime.equals(LocalTime.MAX)))
            return;

        items.add(new ScheduleEventItem(session.getSessionId(), session.getSubjectName(), startTime, endTime, eventColor));
    }
    ...
    public void updateSession(StudySessionEntity session) {
        compositeDisposable.add(
                sessionDao.update(session)
                        ...
                        .subscribe(
                                () -> {
                                    Log.d("SessionAddDialog", "Session updated successfully");
                                },
                                throwable -> {
                                    Log.e("SessionAddDialog", "Error updating session", throwable);
                                }
                        )
        );
    }
    ...
    public void saveSession(StudySessionEntity session) {
        compositeDisposable.add(
                sessionDao.insert(session)
                        ...
                        .subscribe(
                                () -> {
                                    Log.d("SessionAddDialog", "Session saved successfully");
                                },
                                throwable -> {
                                    Log.e("SessionAddDialog", "Error saving session", throwable);
                                }
                        )
        );
    }
